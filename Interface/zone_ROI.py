...
class ZoneRoiInterface(CTkFrame):
    def __init__(self, master, root_width=None, root_height=None, **kwargs):
        ...
        # Camera dropdown - replaced the label with a proper dropdown menu
        ...
        # Refresh button - added to header frame
        ...
        self.refresh_button.pack(side="left", padx=(15, 0), pady=5)
        ...
        self.canvas = tk.Canvas(
            ...
            height=canvas_max_height - 10
        )
        self.canvas.pack(expand=True, fill="both", padx=5, pady=5)

        # self.button_back = CTkButton(
        #     self.canvas,
        #     height=38,
        #     width=100,
        #     text="Back",
        ...
        # )
        #
        # self.button_back.place(relx=0.93, rely=0.01)
        ...
        self.save_button = CTkButton(
            self.button_frame,
            text="Save ROI",
            command=self.save_roi,  # Added missing command binding
            fg_color="#5A616B",
            hover_color="#313A46",
            font=("Arial", 12, "bold"),
            corner_radius=4,
    ...
    def refresh_frame(self):
        """Refresh the current camera frame"""
        if not self.current_camera or not self.rtsp_url:
            self.status_label.configure(text="No camera selected", text_color="#FF0000")
            return

        # Update UI to show loading state
        self.status_label.configure(text="Refreshing frame...", text_color="#FFA500")
        self.update()

        # Temporarily store ROI state
        temp_roi_coords = self.roi_coords
        temp_roi_selected = self.roi_selected

        # Capture a new frame
        self.capture_frame()

        # Restore ROI if it was previously drawn
        if temp_roi_selected and temp_roi_coords:
            self.roi_coords = temp_roi_coords
            self.roi_selected = temp_roi_selected
            self.restore_roi()
            self.save_button.configure(state="normal")
            self.status_label.configure(text="Frame refreshed", text_color="#00FF00")
    ...
    def is_camera_online(self, rtsp_url, timeout=3):
        try:
            command = [
                self.config.get('ffmpeg_lib_path', 'N/A'),
    ...
    def change_camera(self, camera_name):
        ...
        try:
            print("change camera name : ", camera_name)
