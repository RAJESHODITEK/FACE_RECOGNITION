...
class NotificationInterface(CTkFrame):
    def __init__(self, *args, root_width: int = 1920, root_height: int = 1080, **kwargs):
        ...
        self.selected_vehicle_number = None  # Track selected card
    ...
    def on_page_change(self):
        ...
        enabled_color = "#232E51"  # Dark blue when enabled
        enabled_hover = "#1A233C"  # Slightly darker blue on hover
        enabled_text = "#FFFFFF"  # White text

        disabled_color = "#A5B1CD"  # Light blue when disabled
    ...
    def update_button_state(self, button, state, cursor, fg_color, hover_color):
        ...
        button.configure(
            ...
            fg_color="#6073B0" if state == "disabled" else "#232E51",
    ...
    def create_acknowledgment_frame(self,data=None):
        ...
        for label, value in event_details:
            ...
            if label == "Event Type:" and value in ["Alarm", "Alert"]:
                CTkLabel(
                    row_frame,
                    text=value,
                    font=("Inter", 14, "bold"),
        ...
        if vehicle_data.get("vehicle_img"):
            CTkLabel(
                vehicle_section,
                image=vehicle_data["vehicle_img"],
                text=""
            ).pack(pady=5, padx=5)
        ...
        if vehicle_data.get("number_plate_img"):
            CTkLabel(
                vehicle_section,
                image=vehicle_data["number_plate_img"],
                text=""
            ).pack(pady=5, padx=5)
        ...
        print("dataaaa dataaaaaaaaaaaaaaaaaaaaaaaaaaa ", data)
        if data['vehicle_data'] is not None:
            vehicle_details = [
                ("Model Name:", data['vehicle_data'][3]),
                ("Color:", data['vehicle_data'][5]),
                ("Vehicle Type:", data['vehicle_data'][4]),
                ("Mfg Year:", data['vehicle_data'][7]),
                ("Owner Name:", data['vehicle_data'][6])
            ]
        else:
            vehicle_details = [
                ("Model Name:", 'N/A'),
    ...
    def show_acknowledge_dialog(self, data):
    ...
    def update_alarm_list(self, alarm_data_list=None, append=False):
        ...
        for i, data in enumerate(current_page_data, start=start_idx):
            ...
            def create_click_handler(vehicle_event_id, frame, index, vehicle_number):
                def handler(event):
                    ...
                    self.selected_vehicle_number= vehicle_number
            ...
            label_plate = CTkLabel(
                frame_alarm,
                image=data.get("number_plate_img"),
                text="",
            )
