...
from shared_queue import update_any_camera_data, current_camera_details, update_camera_details
...
class CameraRoiInterface(CTkFrame):
    ...
    def is_camera_online(self, rtsp_url, timeout=3):
        try:
            command = [
                r"ffmpeg-7.1-full_build/ffmpeg-7.1-full_build/bin/ffmpeg.exe",
    ...
    def save_roi(self):
        ...
        if self.roi_selected and self.frame is not None:
            try:
                ...
                update_any_camera_data(self.current_camera,'',ROIStartPercentageHeight,ROIEndPercentageHeight,ROIStartPercentageWidth,ROIEndPercentageWidth,'')
                if current_camera_details['camera_name']== self.current_camera:
                    update_camera_details(self.current_camera,current_camera_details['rtsp_url'],ROIStartPercentageHeight,ROIEndPercentageHeight,ROIStartPercentageWidth,ROIEndPercentageWidth,current_camera_details['status'])
