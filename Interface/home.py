...
from shared_queue import all_camera_data, current_camera_details, update_camera_details
...
class HomeInInterface(CTkFrame):
    ...
    def __init__(self, *args, root_width: int = 1920, root_height: int = 1080, **kwargs):
        ...
        self.button_new_option = self.create_menu_buttons(
            self.frame_lcol_mrow,
            ".\\Resources\\images\\setting_icon.png",
            "New Option",
            i_img_width=20,
            i_img_height=20,
            command=self.toggle_settings_popup_new_option

        )
        self.button_new_option_sub1 = self.create_menu_buttons(
            self.frame_lcol_mrow,
        ...
        self.button_new_option_sub1.pack_forget()
        self.button_new_option_sub2 = self.create_menu_buttons(
            self.frame_lcol_mrow,
            ".\\Resources\\images\\camera_manager_icon.png",
            "New Sub Option 2",
            i_img_width=20,
        ...
        self.button_new_option_sub2.pack_forget()

        # self.frame_settings_popup_for_new_option = CTkFrame(
        #     self.frame_lcol_mrow,
        #     width=170,
        #     height=100,
        #     fg_color="#232E51"
        # )
        # self.frame_settings_popup_for_new_option.columnconfigure(0, weight=1)
        # self.frame_settings_popup_for_new_option.grid_propagate(False)
        ...
        self.bool_dropdown_opened = False
        self.bool_settings_popup = False
        self.bool_new_settings_popup = False
        ...
        self.label_alpr_heading.grid(row=0, column=1, sticky="w", padx=0)
    ...
    def toggle_settings_popup_new_option(self):
        # Update the menu button state first
        self.update_menu_buttons_state(self.button_new_option)

        # Always hide settings options first
        self.button_camera_settings.pack_forget()
        self.btn_vehicle_manager.pack_forget()
        self.button_audio_settings.pack_forget()

        if not self.bool_new_settings_popup:
            # Show the new option sub-buttons
            self.button_new_option_sub1.pack(side="top", fill="x", anchor="nw", pady=5, padx=(25, 5))
            self.button_new_option_sub2.pack(side="top", fill="x", anchor="nw", pady=5, padx=(25, 5))
        else:  # Popup is opened, need to close it
            # Hide the buttons
            self.button_new_option_sub1.pack_forget()
            self.button_new_option_sub2.pack_forget()

        # Set the settings popup to closed when using new options
        self.bool_settings_popup = False
        self.bool_new_settings_popup = not self.bool_new_settings_popup

    def toggle_settings_popup(self):
        # Update the menu button state
        self.update_menu_buttons_state(self.button_settings)

        # Always hide the new option sub-buttons
        self.button_new_option_sub1.pack_forget()
        self.button_new_option_sub2.pack_forget()

        # Reset the new settings popup state
        self.bool_new_settings_popup = False
        ...
        self.btn_vehicle_manager.pack_forget()
        self.button_audio_settings.pack_forget()
        self.button_new_option.pack_forget()
        ...
        if not self.bool_settings_popup:  # Popup is closed, need to open it
            ...
            self.button_new_option.pack(side="top", fill="x", anchor="nw", pady=5, padx=(5, 5))
        # If settings popup is already open, we've already hidden the buttons above
    ...
    def select_option(self, selected_option: str, entry_destination: CTkEntry):
        ...
        # Step 1: Set initial state to "Connecting..."
        ...
        entry_destination.configure(state="disabled", border_color="orange")
        ...
        def animate_connecting(dot_count=1):
            if not self.connecting_animation_active:
                return
            ...
            entry_destination.after(500, lambda: animate_connecting(dot_count + 1))
        ...
        def check_connection():
            ...
            if connected:
                entry_destination.after(0, update_to_connected)
            else:
                entry_destination.after(0, update_to_failed)

        def update_to_connected():
            entry_destination.configure(state="normal", text_color="#008000", border_color="#008000")  # Green border
            entry_destination.delete(0, "end")
            entry_destination.insert(0, "Connected")
            entry_destination.configure(state="disabled")
            self.button_select_camera.configure(state="normal")
            entry_destination.after(1000, update_to_camera_name)


        def update_to_failed():
            entry_destination.configure(state="normal", text_color="red", border_color="red")  # Red border for failure
            entry_destination.delete(0, "end")
            entry_destination.insert(0, "Connection Failed")
            entry_destination.configure(state="disabled")
...
#----------------------------------------------------------------------------------
    ...
    def toggle_menu_bar(self):
        if (self.bool_toggle_menu_bar == True):
            ...
            # self.button_vehicle_list.configure(text="", anchor="center")
           ...
           # self.camera_manager.configure(text="", anchor="center")
            ...
            # self.button_vehicle_list.configure(text="Vehicle List", anchor="w")
