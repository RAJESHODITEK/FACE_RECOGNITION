...
class HomeInInterface(CTkFrame):
    ...
    def select_option(self, selected_option: str, entry_destination: CTkEntry):
        ...
        current_camera_details = next((cam for cam in all_camera_data if cam["camera_name"] == selected_option),
                                      None)
        print("selected camera details :",current_camera_details)
        update_camera_details(current_camera_details['camera_name'],current_camera_details['rtsp_url'],current_camera_details['roi_start'],current_camera_details['roi_end'],current_camera_details['status'])
        self.frame_maindropdown_window.place_forget()
        self.bool_dropdown_opened = False
