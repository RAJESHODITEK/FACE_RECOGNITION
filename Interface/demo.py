...
#             self.frame_filter.grid_propagate(False)
#             self.frame_filter.grid(row=2, column = 0, rowspan = 2, sticky = "ne", padx=19, pady=(2,0))
...
#     # def on_next_prev_click(self):
#     #     vehicle_number = self.vehicle_data.get("vehicle_number", "") 
#     #     for vehicle in self.vehicle_data:
#             # print(vehicle,"*********")
#             # print("heloo")
#     #         if vehicle in self.selected_vehicle_set:
#     #             # This vehicle is selected, so we check its checkbox in the listveh
#     #             try:
#     #                 # Assuming the checkboxes are linked to the row index
#     #                 checkbox = self.frame_table_rows.grid_slaves(row=self.vehicle_data.index(vehicle) * 2, column=5)[0]
#     #                 checkbox.select()  # Check the checkbox for the selected vehicle
#     #             except Exception as e:
#     #                 print(f"Error selecting checkbox for vehicle {vehicle}: {e}")
#     #         else:
#     #             # This vehicle is not selected, so we uncheck its checkbox
#     #             try:
#     #                 checkbox = self.frame_table_rows.grid_slaves(row=self.vehicle_data.index(vehicle) * 2, column=5)[0]
#     #                 checkbox.deselect()  # Uncheck the checkbox for the unselected vehicle
#     #             except Exception as e:
#     #                 print(f"Error deselecting checkbox for vehicle {vehicle}: {e}")

#     # # After processing the selections, update button states (enable/disable based on selection)
#     #     self.update_button_states()

#     def reset_checkbox(self, bool_isPageChanged : bool = False):
#         if not bool_isPageChanged:
...
#         self.list_blacklist=["black-list","white-list"]
...
#     def popup_Add_dropdown(self, parent_frame: CTkFrame, list_data: list = [], entry_destination: CTkEntry = None, i_row: int = None, i_rowspan: int = 3, type: int = 0):
...
#             else:
#                 # If the entry is empty, show all options
...
#                     hover_color="#F6F6F6",
#                     anchor="w",
#                     command=lambda selected_option=row_data: self.select_option(selected_option, entry_destination, frame_Add_maindropdown_window)
#                 )
...
#                 # Show the dropdown
#                 frame_Add_maindropdown_window.grid_propagate(False)
#                 frame_Add_maindropdown_window.grid(row=i_row, column=0, rowspan=i_rowspan, sticky="nsew", padx=(14, 7), pady=(5, 0))
...
#         print("hello",len(self.selected_vehicle_set))
...
#                 height = 35,
#                 fg_color = "#F6F6F6",
#                 textvariable = StringVar(value="white-list" if vehicle["vehicle_status"] == 0 else "black-list"),
#                 text_color="#414141",
#                 border_color = "#DEDEDE",
#                 border_width = 2,
#                 corner_radius = 5,
