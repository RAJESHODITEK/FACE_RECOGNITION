...
class DeleteVehicleInteface(CTkFrame):
    ...
    def __init__(self, *args, root_width : int = 1920, root_height : int = 1080, **kwargs):
        ...
        self.frame_vehicle_label = CTkFrame(
            self.frame_form,
            height=30,
            fg_color="transparent"  
        )
        self.frame_vehicle_label.grid(row=0, column=0, columnspan=2, padx=20, pady=(20,0), sticky="ew")

        self.label_vehicle = CTkLabel(
            self.frame_vehicle_label,
            text="Vehicle",
            font=("", 14),
            text_color="#2C2C2C"
        )
        self.label_vehicle.pack(side="left")

        self.label_vehicle_error = CTkLabel(
            self.frame_vehicle_label,
            text="",
            text_color="#FF0000",
            font=("", 12),
        )
        self.label_vehicle_error.pack(side="left", padx=(10,0))

        self.frame_vehicle_dropdown = CTkFrame(
            self.frame_form,
            height=40,
            fg_color="#F6F6F6",
            corner_radius=5,
        )
        self.frame_vehicle_dropdown.columnconfigure(0, weight = 1)
        self.frame_vehicle_dropdown.rowconfigure(0, weight = 1)
        self.frame_vehicle_dropdown.grid(row=1, column=0, columnspan=2, padx=20, sticky="ew")

        self.entry_selected_vehicle = CTkEntry(
            self.frame_vehicle_dropdown,
            height = 40,
            fg_color = "#F6F6F6",
    ...
    def update_on_input_changed(self, entry_input_field : CTkEntry = None, label_error : CTkLabel = None, str_error : str = "", str_border_color : str = None):
        if(entry_input_field is not None):
            ...
            if(label_error is None):
                if entry_input_field == self.entry_selected_vehicle:
                    label_error = self.label_vehicle_error
    ...
    def reset_interface(self):
        self.entry_selected_vehicle.configure(state="normal", text_color = "#828282", border_color = "#DEDEDE")
        self.entry_selected_vehicle.delete(0, "end")
        self.entry_selected_vehicle.insert(0, "Select an option")
        ...
        self.label_vehicle_error.configure(text="")
