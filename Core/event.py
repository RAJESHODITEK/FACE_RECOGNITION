...
class Event():
    ...
    def fetch_events(self, i_start_index: int, dict_filter_criteria: dict):
        ...
        try:
            ...
            # Define base query (Ensuring correct column names)
            base_query = f"""
                SELECT event_id, person_name, person_img, captured_img, 
                       start_time, end_time, age, gender, person_type, 
                       acknowledgment_time, acknowledgment_message
                FROM [{self.dict_db_details['str_db_name']}].[dbo].[{self.dict_db_details['str_event_details_table']}]
            """

            # Determine filtering conditions
            if not dict_filter_criteria["str_start_timeperiod"] and not dict_filter_criteria[
                "str_end_timeperiod"] and not dict_filter_criteria["str_vehicle_number"]:
                query = base_query + " ORDER BY start_time DESC OFFSET ? ROWS FETCH NEXT 10 ROWS ONLY"
                cursor.execute(query, i_start_index)
            else:
                query = base_query + """
                    WHERE start_time >= ? AND end_time <= ? 
                    AND person_name LIKE ?
                    ORDER BY start_time DESC OFFSET ? ROWS FETCH NEXT 10 ROWS ONLY
                """
                cursor.execute(query,
                               dict_filter_criteria["str_start_timeperiod"],
                               dict_filter_criteria["str_end_timeperiod"],
                               dict_filter_criteria["str_vehicle_number"],
                               i_start_index)

            response = cursor.fetchall()

            if response:
                columns = [column[0] for column in cursor.description]  # Fetch column names dynamically
                for data in response:
                    if data.person_img != "N/A" and data.captured_img != "N/A":
                        record_dict = {columns[i]: data[i] for i in range(len(columns))}

                        # Format datetime fields
                        for key in ["start_time", "end_time", "acknowledgment_time"]:
                            if key in record_dict and isinstance(record_dict[key], (
                            str, bytes)):  # Ensure it's a datetime object before replacing microseconds
                                record_dict[key] = str(record_dict[key]).split('.')[0]

                        list_event.append(record_dict)

                for data in list_event:
                    data["person_img"] = self.base64_to_cv2mat_or_pillow_image_converter(data["person_img"], 200, 135)
                    data["captured_img"] = self.base64_to_cv2mat_or_pillow_image_converter(data["captured_img"], 180,
                                                                                           50)

                # Fetch the earliest event date using 'start_time' instead of 'time'
                query = f"""
                    SELECT TOP 1 FORMAT(start_time, 'dd-MM-yyyy')
                    FROM [{self.dict_db_details['str_db_name']}].[dbo].[{self.dict_db_details['str_event_details_table']}]
    ...
    def fetch_event_combo_details(self, i_start_index: int, dict_filter_criteria: dict):
        list_events = []
        ...
        try:
            ...
            # Define base query for event_details and personregister join
            base_query = f"""
                   SELECT 
                       e.event_id,
                       e.person_name,
                   ...
                   JOIN [{self.dict_db_details['str_db_name']}].[dbo].[personregister] p
                   ON e.person_name = p.full_name
               """
            ...
            if dict_filter_criteria.get("str_gender"):
            ...
            print(f"Error in fetch_event_details: {e}")
    ...
    def fetch_unrecognized_vehicles(self):
        ...
        try:
            ...
            for data in list_unrecognized_event:
                data["person_img"] = self.base64_to_cv2mat_or_pillow_image_converter(data["person_img"], 200, 130)
