...
class LicensePlateRecognizer:
    ...
    def process_vehicle_data(self, vehicle_data, connection):
        ...
        try:
            ...
            for i in range(1, 6):
                img_base64 = vehicle_data.get(f'vehicle_img{i}', '').strip()
                if not img_base64:
                    continue
            ...
            if not detection_result or detection_result.get('plate_number') == 'N/A':
                ...
                logging.info(f"Default 'N/A' data inserted for vehicle {vehicle_data.get('vehicle_id', 'N/A')}")

                connection.commit()

        except Exception as e:
            logging.error(f"Error processing vehicle {vehicle_data.get('vehicle_id', 'N/A')}: {str(e)}", exc_info=True)
