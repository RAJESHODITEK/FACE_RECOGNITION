...
from FlowControl.camera_roi import CameraRoiController
from FlowControl.home import HomeController
from Interface.main import Interface
from shared_queue import current_camera_details, update_camera_details, update_any_camera_data
...
class CameraManagerController:
    def __init__(self, Core: Core, Interface: Interface):
        ...
        self.camera_manager_interface.button_roi.configure(command=self.on_click_roi)
    ...
    def on_click_roi(self):
            ...
            self.obj_CameraRoiController.load_cameras_from_db()
            self.obj_Interface.dict_frames["camera_roi"].change_camera(list(self.camera_manager_interface.selected_cameras)[0])
            self.obj_Interface.switch_frames('camera_roi')
    ...
    def onclick_save(self) -> None:
        ...
        if eligible_to_save:
            ...
            if self.camera_manager_interface.btn_save.cget('text')=='Save':
                for data in self.camera_manager_interface.dummy_camera_details:
                    # print("camera data is ", data)
                    if self.camera_manager_interface.entry_camera_name.get() == data.get('name', 'N/A'):
                        self.obj_Interface.on_error(
                            "home",
                            "Add operation Failed",
                            "Camera is already present.",
                            "#FF4B4B"
                        )
                        return
            ...
            if check_sucess:
                ...
                if self.camera_manager_interface.btn_save.cget('text')=="Save":
                    ...
                    if self.old_camera_data_during_edit.get('direction','N/A')== self.camera_manager_interface.entry_selected_direction.get() and self.old_camera_data_during_edit.get('url','N/A')== self.camera_manager_interface.entry_rtsp_url.get():
                        self.obj_Interface.on_error(
                            "home",
                            "Update Failed",
                        ...
                        self.obj_Interface.on_error(
                            ...
                            "Your updated details are reflected now .",
                            "#63CA6D"
                        )
                        update_any_camera_data(camera_name=camera_name,rtsp_url=rtsp_url,camera_status=direction)
                        if current_camera_details['camera_name'] == camera_name:
                            update_camera_details(camera_name, rtsp_url,camera_status= current_camera_details['status'])
                        self.onclick_cancel()
                        # self.camera_manager_interface.selected_cameras.clear()
                        self.camera_manager_interface.reset_interface()
                        self.update_camera_data_list()
