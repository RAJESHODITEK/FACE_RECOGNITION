...
class HistoricalEventController:
    ...
    def bind_label_image(self):
        if self.obj_HistoricalEventInterface.current_person_id != self.prev_id:
            self.obj_Interface.dict_frames["notification"].selected_person = self.obj_HistoricalEventInterface.current_person_id
            self.obj_HistoricalEventInterface.show_acknowledge_dialog(self.obj_core.obj_person.fetch_single_person_data(self.obj_HistoricalEventInterface.current_person_number)['person_data'],personimg=self.obj_HistoricalEventInterface.current_person_image, capturedimg=self.obj_HistoricalEventInterface.current_captured_image,event_data=self.obj_HistoricalEventInterface.current_event_details, eventType=self.obj_HistoricalEventInterface.eventType)
            self.obj_HistoricalEventInterface.current_person_id=""
    ...
    def bind_buttons(self):
        ...
        self.obj_HistoricalEventInterface.button_select_person.configure(command=self.popup_person_dropdown)
        self.obj_HistoricalEventInterface.button_select_status.configure(command=self.popup_status_dropdown)
    ...
    def bind_add_popup_buttons(self):
        ...
        try:
            if (hasattr(self.obj_HistoricalEventInterface, 'add_person_button') and
                    self.obj_HistoricalEventInterface.add_person_button is not None):
                # Unbind any existing command first
                self.obj_HistoricalEventInterface.add_person_button.configure(command=None)
                # Rebind the command
                self.obj_HistoricalEventInterface.add_person_button.configure(command=self.onclick_ack_add_person)

        except Exception as e:
           pass

    def onclick_ack_add_person(self):
        try:
            # Add the person
            self.obj_Interface.dict_frames["person_list"].add_Vechile(
                num=self.obj_HistoricalEventInterface.current_person_number
            )
            ...
            print(f"person {self.obj_HistoricalEventInterface.current_person_number} added successfully")
    ...
    def popup_status_dropdown(self):
        ...
        elif self.obj_HistoricalEventInterface.bool_person_dropdown_opened:
            self.obj_HistoricalEventInterface.close_dropdown(None)
            self.obj_HistoricalEventInterface.bool_person_dropdown_opened = False

        if self.obj_HistoricalEventInterface.bool_status_dropdown_opened:
            self.obj_HistoricalEventInterface.close_dropdown(None)
            self.obj_HistoricalEventInterface.bool_status_dropdown_opened = False
        else:
            # Define status options (these could come from your backend or be hardcoded)
            status_options = ["Unknown", "Verified", "Restricted","All"]
            self.obj_HistoricalEventInterface.popup_dropdown(
                list_data=status_options,
                entry_destination=self.obj_HistoricalEventInterface.entry_selected_status,
                i_row=5,
                i_rowspan=6
            )
            self.obj_HistoricalEventInterface.bool_status_dropdown_opened = True
    ...
    def popup_person_dropdown(self):
        if (self.obj_HistoricalEventInterface.bool_sdate_dropdown_opened):
            self.obj_HistoricalEventInterface.close_dropdown(None)
            self.obj_HistoricalEventInterface.bool_sdate_dropdown_opened = False
        elif (self.obj_HistoricalEventInterface.bool_edate_dropdown_opened):
        ...
        if (self.obj_HistoricalEventInterface.bool_person_dropdown_opened is True):
            self.obj_HistoricalEventInterface.close_dropdown(None)
            self.obj_HistoricalEventInterface.bool_person_dropdown_opened = False
        else:
            list_person = self.obj_core.obj_event.get_distinct_persons()
            self.obj_HistoricalEventInterface.popup_dropdown(
                list_person,
                entry_destination=self.obj_HistoricalEventInterface.entry_selected_person,
                i_row=6,
                i_rowspan=3)
            self.obj_HistoricalEventInterface.bool_person_dropdown_opened = True

    def popup_status_dropdown(self):
        # Close other dropdowns
        for flag in [
            'bool_sdate_dropdown_opened',
            'bool_edate_dropdown_opened',
            'bool_person_dropdown_opened'
        ]:
            if getattr(self.obj_HistoricalEventInterface, flag):
                self.obj_HistoricalEventInterface.close_dropdown(None)
                setattr(self.obj_HistoricalEventInterface, flag, False)
        ...
        if self.obj_HistoricalEventInterface.bool_status_dropdown_opened:
            ...
            status_options = self.obj_core.obj_event.get_distinct_statuses()

            # Add "All" option at the beginning
            status_options.insert(0, "All")

            # Make sure we have at least the default options if the database query fails
            if not status_options or len(status_options) <= 1:
                status_options = ["All", "Unknown", "Verified", "Restricted"]
    ...
    def onclick_ok(self):
        ...
        str_person_number = self.obj_HistoricalEventInterface.entry_selected_person.get()
        str_status = self.obj_HistoricalEventInterface.entry_selected_status.get()
        ...
        dict_response = self.obj_core.obj_event.validate_filter_criteria(str_start_date, str_end_date,
                                                                         ...
                                                                         str_person_number,'',str_status)

        print(f" data of filter :  \n{dict_response["dict_filter_criteria"]}\n")
        ...
        if (dict_response["str_error_msg_heading"] == "" and dict_response["str_error_msg"] == ""):
            ...
            list_historical_events, self.obj_Interface.dict_frames[
                ...
                self.obj_HistoricalEventInterface.i_start_index, dict_response["dict_filter_criteria"])

            i_total_data_fetched = len(list_historical_events)
            self.obj_HistoricalEventInterface.i_total_data= self.obj_core.obj_event.get_data_count(dict_response["dict_filter_criteria"])
    ...
    def onclick_next(self):
        ...
        if hasattr(self.obj_HistoricalEventInterface, 'ack_window') and self.obj_HistoricalEventInterface.ack_window:
            self.obj_HistoricalEventInterface.ack_window.destroy()
        self.obj_HistoricalEventInterface.button_next.configure(state='disabled')


        self.obj_HistoricalEventInterface.i_start_index =  self.obj_HistoricalEventInterface.i_start_index + 5
        self.obj_HistoricalEventInterface.i_end_index = self.obj_HistoricalEventInterface.i_start_index + 5
        print(f"###################______________data from {self.obj_HistoricalEventInterface.i_start_index} to {self.obj_HistoricalEventInterface.i_start_index+5}")
    ...
    def onclick_previous(self):\
        ...
        if hasattr(self.obj_HistoricalEventInterface, 'ack_window') and self.obj_HistoricalEventInterface.ack_window:
            self.obj_HistoricalEventInterface.ack_window.destroy()
        self.obj_HistoricalEventInterface.button_previous.configure(state='disabled')
    ...
    def count_entry_exit_balance(self,event=None, data=[]):
        ...
        if data ==[]:
            ...
            str_end_date = self.obj_HistoricalEventInterface.entry_selected_edate.get()
            str_start_hour = self.obj_HistoricalEventInterface.spinbox_shour.get()
            str_end_hour = self.obj_HistoricalEventInterface.spinbox_shour.get()
            str_start_minute = self.obj_HistoricalEventInterface.spinbox_sminute.get()
            str_end_minute = self.obj_HistoricalEventInterface.spinbox_sminute.get()
            str_person_number = self.obj_HistoricalEventInterface.entry_selected_person.get()
        else:
        ...
        try:
            ...
            if str_person_number == "Select person" or str_person_number == "All":
                str_person_number='%'
        ...
        dict_filter_criteria = {
            ...
            "str_person_number": str_person_number
