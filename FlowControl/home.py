...
from shared_queue import all_camera_data
...
class HomeController:
    ...
    def set_camera_name(self):
        ...
        try :
            ...
            if count > 0 and camera_data:
                ...
                self.obj_PTZController.change_ptz(camera_data[0].get('URL', 'N/A'))
    ...
    def on_minimize(self, event=None):
        ...
        try:
            ...
            if 'person_list' in self.obj_Interface.dict_frames and hasattr(
                    self.obj_Interface.dict_frames['person_list'], 'popup'):
                self.obj_Interface.dict_frames['person_list'].popup.iconify()
            else:
                logging.warning("'person_list' or 'popup' attribute not found during minimize operation.")
        except Exception as e:
            logging.error(f"An error occurred during minimize: {e}")
    ...
    def on_window_restored(self, event=None):
        ...
        try:
            # Check if 'person_list' exists in the dictionary and has a 'popup' attribute
            if 'person_list' in self.obj_Interface.dict_frames and hasattr(
                    self.obj_Interface.dict_frames['person_list'], 'popup'):
                self.obj_Interface.dict_frames['person_list'].popup.deiconify()
            else:
                logging.warning("'person_list' or 'popup' attribute not found during restore operation.")
        except Exception as e:
            logging.error(f"An error occurred during restore: {e}")
    ...
    def bind_buttons(self) -> None:
        ...
        # self.obj_HomeInterface.button_add_person.configure(
        #     command=lambda: self.onclick_menu_buttons(self.obj_HomeInterface.button_add_person))
        # self.obj_HomeInterface.button_person_list.configure(
        #     command=lambda: self.onclick_menu_buttons(self.obj_HomeInterface.button_person_list))
        # self.obj_HomeInterface.button_delete_person.configure(
        #     command=lambda: self.onclick_menu_buttons(self.obj_HomeInterface.button_delete_person))
    ...
    def bind_settings_buttons(self) -> None:
        ...
        if hasattr(self.obj_HomeInterface, 'btn_person_manager'):
            self.obj_HomeInterface.btn_person_manager.configure(
                command=lambda: self.onclick_submenu_button("person_list")
            )
    ...
    def onclick_submenu_button(self, option: str) -> None:
        ...
        self.obj_Interface.dict_frames["add_person"].reset_interface()
        self.obj_Interface.dict_frames['person_list'].close_webcam()
        self.obj_Interface.dict_frames["delete_person"].reset_interface()
        self.obj_Interface.dict_frames["person_list"].reset_interface()
        self.obj_Interface.dict_frames["historical_event"].reset_interface()
        self.obj_Interface.dict_frames['camera_manager'].reset_interface()
        self.obj_Interface.dict_frames['person_list'].reset_checkbox()
        self.obj_Interface.dict_frames['person_list'].selected_id.clear()
        ...
        if option == "camera":
        ...
        elif option == 'person_list':
            self.obj_HomeInterface.update_menu_buttons_state(self.obj_HomeInterface.btn_person_manager)
            self.obj_Interface.dict_frames["person_list"].dict_filter_criteria = {
                "gender": "%",
                "status": "%",
                "full_name": "",
                'str_person_number': 0
            ...
            self.obj_Interface.dict_frames["person_list"].person_data = self.obj_Core.obj_person.fetch_person_details(
                self.obj_Interface.dict_frames["person_list"].dict_filter_criteria)
            self.obj_Interface.dict_frames["person_list"].i_total_data = len(
                self.obj_Interface.dict_frames["person_list"].person_data)
            print(self.obj_Interface.dict_frames["person_list"].i_total_data,
                  "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
            if (self.obj_Interface.dict_frames["person_list"].i_total_data > 0):
                self.obj_Interface.dict_frames["person_list"].i_start_index = 1
                self.obj_Interface.dict_frames["person_list"].i_end_index = 5 if self.obj_Interface.dict_frames[
                                                                                     "person_list"].i_total_data >= 5 else \
                self.obj_Interface.dict_frames["person_list"].i_total_data
            first_five_vehicles = self.obj_Interface.dict_frames["person_list"].person_data[:5]
            self.obj_Interface.dict_frames["person_list"].update_table(first_five_vehicles)
    ...
    def onclick_menu_buttons(self, button_menu) -> None:
        ...
        # if (button_menu == self.obj_HomeInterface.button_add_person):
        #     self.obj_Interface.switch_frames("add_person")
        # elif (button_menu == self.obj_HomeInterface.button_delete_person):
        #     self.obj_Interface.switch_frames("delete_person")
        if (button_menu == self.obj_HomeInterface.button_edit):
    ...
    def onclick_notification_button(self):
        ...
        print("Calling fetch_unrecognized_persons...")
        db_data_list = self.obj_Core.obj_event.fetch_unrecognized_persons()
        print(f"Received {len(db_data_list) if db_data_list else 0} records from fetch_unrecognized_persons")

        # Update person data in interface
        if db_data_list:
            ...
            self.obj_Interface.dict_frames['notification'].person_data = temp_notification_data
