...
class HomeController:
    ...
    def on_minimize(self, event=None):
        ...
        try:
            ...
            if 'historical_event' in self.obj_Interface.dict_frames and hasattr(
                    self.obj_Interface.dict_frames['historical_event'], 'popup'):
                # print("type ",type(self.obj_Interface.dict_frames['historical_event'].popup))
                self.obj_Interface.dict_frames['historical_event'].popup.iconify()
            else:
               pass
            #  logging.warning("'historical_event' or 'popup' attribute not found during minimize operation.")
        except Exception as e:
            pass
            # logging.error(f"An error occurred during minimize: {e}")
    ...
    def bind_buttons(self) -> None:
        ...
        self.obj_HomeInterface.button_notification.configure(
            command=lambda: self.onclick_menu_buttons(self.obj_HomeInterface.button_notification))
    ...
    def onclick_submenu_button(self, option: str) -> None:
        ...
        if option == "camera":
            ...
            # self.obj_Interface.dict_frames["vehicle_list"].vehicle_data = self.obj_Core.obj_Vehicle.fetch_vehicle_details(self.obj_Interface.dict_frames["vehicle_list"].i_end_index,self.obj_Interface.dict_frames["vehicle_list"].dict_filter_criteria)
            ...
            # self.obj_Interface.dict_frames["vehicle_list"].reset_filter_criteria()
            self.obj_Interface.dict_frames["vehicle_list"].vehicle_data = self.obj_Core.obj_Vehicle.fetch_person_details(self.obj_Interface.dict_frames["vehicle_list"].dict_filter_criteria)
            self.obj_Interface.dict_frames["vehicle_list"].i_total_data = len(self.obj_Interface.dict_frames["vehicle_list"].vehicle_data)
            print(self.obj_Interface.dict_frames["vehicle_list"].i_total_data,"^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
            ...
            first_five_vehicles = self.obj_Interface.dict_frames["vehicle_list"].vehicle_data[:5]
            self.obj_Interface.dict_frames["vehicle_list"].update_table(first_five_vehicles)
            self.obj_Interface.switch_frames("vehicle_list")
    ...
    def onclick_menu_buttons(self, button_menu) -> None:
        ...
        # elif (button_menu == self.obj_HomeInterface.button_vehicle_list):
        #     self.obj_Interface.dict_frames["vehicle_list"].reset_filter_criteria()
        #     self.obj_Interface.dict_frames["vehicle_list"].vehicle_data = self.obj_Core.obj_Vehicle.fetch_vehicle_details(self.obj_Interface.dict_frames["vehicle_list"].i_end_index,
        #                                                                                                                     self.obj_Interface.dict_frames["vehicle_list"].dict_filter_criteria)
        #     self.obj_Interface.dict_frames["vehicle_list"].i_total_data = self.obj_Core.obj_Vehicle.get_data_count(self.obj_Interface.dict_frames["vehicle_list"].dict_filter_criteria)
        #     i_total_data_fetched = len(self.obj_Interface.dict_frames["vehicle_list"].vehicle_data)
        #     if(i_total_data_fetched > 0):
        #         self.obj_Interface.dict_frames["vehicle_list"].i_start_index = 1
        #         self.obj_Interface.dict_frames["vehicle_list"].i_end_index += i_total_data_fetched
        #     self.obj_Interface.dict_frames["vehicle_list"].update_table(self.obj_Interface.dict_frames["vehicle_list"].vehicle_data)
        #     self.obj_Interface.switch_frames("vehicle_list")
        ...
        elif (button_menu == self.obj_HomeInterface.button_notification):
